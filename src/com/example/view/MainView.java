...
public class MainView extends BaseView{
	...
	public void drawSelf() {
		...
		try {
			...
			if(!myPlane.isAlive()){
				threadFlag = false;
				sounds.playSound(4, 0,isSound);			//飞机炸毁的音效
			}
			myPlane.drawSelf(canvas);	//绘制玩家的飞机
			myPlane.shoot(canvas,enemyPlanes);
			sounds.playSound(1, 0,isSound);	  //子弹音效		
			//绘制导弹按钮
			if(missileCount > 0){
				paint.setTextSize(40);
			...
			paint.setColor(Color.rgb(80, 80, 80));
			paint.setAntiAlias(true);
			canvas.drawText("当前分数: "+String.valueOf(sumScore), 20 + bt_w, 50, paint);		//绘制文字
			//canvas.drawText("等级 X "+String.valueOf(speedTime), screen_width - 150, 30, paint); //绘制文字
		} catch (Exception err) {
			err.printStackTrace();
		} finally {
			if (canvas != null)
