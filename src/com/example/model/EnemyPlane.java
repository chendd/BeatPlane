...
public class EnemyPlane extends GameObject{
	...
	public boolean isCollide(GameObject obj) {
		...
		if (object_x <= obj.getObject_x()
				&& object_x + object_width -30<= obj.getObject_x()) {
			return false;
		}
		// 矩形1位于矩形2的右侧
		else if (obj.getObject_x() <= object_x
				&& obj.getObject_x() + obj.getObject_width() <= object_x+30 ) {
			return false;
		}
		// 矩形1位于矩形2的上方
		else if (object_y <= obj.getObject_y()
				&& object_y + object_height -25 <= obj.getObject_y()) {
			return false;
		}
		// 矩形1位于矩形2的下方
		else if (obj.getObject_y() <= object_y
				&& obj.getObject_y() + obj.getObject_height() <= object_y +25) {
			return false;
		}
		return true;
	}
